        ...
        for (const item of items) {
            try {
                ...
                if (item.department || item.category) {
                    const deptName = item.department || item.category
                ...
                if (existing) {
                    if (updateExisting) {
                        // Update existing item - preserve stock from CSV (migration)
                        await prisma.item.update({
                            where: { id: existing.id },
                            data: {
                                name: item.enrichedName || item.originalName || existing.name,
                                brand: item.brand || existing.brand,
                                size: item.size || existing.size,
                                cost: item.cost || existing.cost,
                                price: item.price || existing.price,
                                stock: item.stock, // Use CSV stock for migration
                    ...
                    // Create new item with stock from CSV
                    await prisma.item.create({
                        data: {
                            ...
                            cost: item.cost || 0,
                            price: item.price || 0,
                            stock: item.stock || 0, // Stock from old system
                            categoryId,
