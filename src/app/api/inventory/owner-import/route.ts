...
export async function POST(request: NextRequest) {
    try {
        ...
        })).filter(item => item.upc || item.originalName)
        ...
        return NextResponse.json({
            ...
            headers,
            columnMappings,
            items: items.slice(0, 500) // Return up to 500 for preview
        })
...
function detectColumnMappings(headers: string[]) {
    ...
    return {
        upc: findHeader(['upc', 'barcode', 'code', 'ean', 'gtin', 'plu', 'sku']),
        ...
        stock: findHeader(['qty', 'stock', 'quantity', 'on hand', 'count', 'inventory', 'oh'])
    }
}
