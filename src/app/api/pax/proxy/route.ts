...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!isPrivateIP(ip)) {
            console.error(`[PAX Proxy] BLOCKED: Attempt to connect to non-private IP: ${ip}`)
            return NextResponse.json(
        ...
        console.error('[PAX Proxy] Error:', error.message)
        return NextResponse.json(
...
function sendTcpRequest(ip: string, port: string, data: Buffer): Promise<Buffer> {
    return new Promise((resolve, reject) => {
        ...
        client.on('error', (err) => {
            console.error('[PAX Proxy] Socket error:', err.message)
            reject(err)
        })
    })
}
