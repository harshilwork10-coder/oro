...
if (!user.franchiseId) {
        ...
        // ─── Tobacco Scan Deal Auto-Apply ───────────────────────────────
        // Uses the dedicated TobaccoScanDeal engine for manufacturer-funded
        // tobacco deals. Matches by UPC barcode via TobaccoScanDealUPC.
        // Customer sees the discount in cart. Scan events are created
        // post-checkout (not here).
        try {
                ...
                const cartUpcs = products
                    .map(p => p.barcode)
                    .filter((b): b is string => !!b)

                if (cartUpcs.length > 0) {
                    // Find active scan deals with matching UPCs
                    const scanDeals = await prisma.tobaccoScanDeal.findMany({
                        where: {
                            franchiseId: user.franchiseId,
                            status: 'ACTIVE',
                            startDate: { lte: now },
                    ...
                    for (const deal of scanDeals) {
                        const dealUpcs = new Set(deal.eligibleUpcs.map(u => u.upc))

                        // Find cart items that match this deal's UPCs
                        const matchingItems = items.filter(item => {
                            const product = products.find(p => p.id === item.id)
                            return product?.barcode && dealUpcs.has(product.barcode) &&
                                (!usedItemIds.has(item.id) || deal.stackable)
                        })

                        if (matchingItems.length === 0) continue

                        const totalQty = matchingItems.reduce((s, i) => s + i.quantity, 0)
                        if (totalQty < deal.minQty) continue
                        if (deal.maxQty !== null && totalQty > deal.maxQty) continue

                        let discountAmount = 0
                        const affectedItemIds = matchingItems.map(i => i.id)
                        const rewardValue = Number(deal.rewardValue)

                        switch (deal.rewardType) {
                            case 'FIXED_AMOUNT':
                                discountAmount = rewardValue * totalQty
                                break
                            case 'PERCENTAGE':
                                for (const item of matchingItems) {
                                    discountAmount += item.price * item.quantity * (rewardValue / 100)
                                }
                                break
                            case 'PENNY_DEAL': {
                                const sorted = [...matchingItems].sort((a, b) => a.price - b.price)
                                const pennyQty = Math.min(totalQty, deal.maxQty || totalQty)
                                for (const item of sorted) {
                                    const qty = Math.min(item.quantity, pennyQty)
                                    discountAmount += (item.price - 0.01) * qty
                                }
                                break
                            }
                            case 'FREE_UNIT': {
                                if (totalQty >= deal.minQty) {
                                    const freeQty = Math.floor(totalQty / deal.minQty)
                                    const sorted = [...matchingItems].sort((a, b) => a.price - b.price)
                                    discountAmount = sorted[0]?.price * freeQty || 0
                                }
                                break
                            }
                            default:
                        ...
                        if (discountAmount > 0) {
                            appliedPromotions.push({
                                promotionId: `tobacco-scan-${deal.id}`,
                ...
                // Don't break checkout if tobacco scan deals fail
                console.error('[TOBACCO_SCAN_DEALS_CHECK]', tobaccoError)
            }
