        ...
        <div className="min-h-screen bg-gradient-to-br from-stone-950 via-stone-900 to-stone-950 text-white p-6">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div><h1 className="text-3xl font-bold flex items-center gap-2"><Activity className="h-8 w-8 text-emerald-500" /> Live Sales</h1><p className="text-stone-400">Auto-refreshes every 60 seconds</p></div>
                </div>
                <button onClick={refresh} className="px-4 py-2 bg-stone-700 rounded-xl hover:bg-stone-600"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></button>
            </div>
                    ...
                    <div className="text-center mb-8">
                        ...
                        <div className="flex items-center justify-center gap-2 mt-3">
                            ...
                            <span className={`text-lg font-semibold ${(data.vsYesterday ?? 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                {data.vsYesterday > 0 ? '+' : ''}{data.vsYesterday?.toFixed(1) || 0}% vs yesterday
                            </span>
                        </div>
                    </div>
